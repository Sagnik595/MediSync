  ...
  const handlecancel = async (appointmentID) => {
    try {
      ...
      if (data.success) {
        ...
        // Option 1: Refetch from backend (recommended)
        await loadAppointments();

        // Option 2: Local update only
        // setAppointment(
        //   bookappointment.filter(
        //     (item) => item._id.toString() !== appointmentID.toString()
        //   )
        // );
      } else {
      ...
      <div className="px-10 mt-25 mb-16">
        ...
        <div className="divide-y divide-gray-200">
          {bookappointment.map((item) => (
            <div key={item._id} className="flex items-start gap-6 py-6">
              {/* Image */}
              <img
